...
module.exports = function(RED) {
    ...
    function PlayerNode(n) {
        ...
        this.on('input', function(msg) {
            ...
            if (msg.hasOwnProperty('pause')) {
                if (_debugmode) {
                    _node.warn("pause received");
                }
                triggerCommand('pause', msg.pause);
            }
    ...
    RED.nodes.registerType("recorder", RecoderNode);
    ...
    function SequencePlayer(n) {
        ...
        function resume(elapsedTime) {
            if (elapsedTime && hasSequence() && _currentElement) {
                if (elapsedTime === 0) {
                    ...
                    var timer = setTimeout(function() {
                        ...
                        _node.send(_currentElement.data);
                        run();
                    }, _currentElement.delay - elapsedTime);
                    _lastTimer = timer;
                }
            }
        }
    ...
    function SequenceRecorder(n) {
        ...
        this.start = function(msg) {
            if (!_isRecording) {
                ...
                if (_startImmediately) {
                    _prevReceiveTime = new Date().getTime();
        ...
        this.recordElement = function(msg) {
            ...
            if (_isRecording) {
                ...
                var delay = _util.calcDelay(_prevReceiveTime, receiveTime);
    ...
    function Util() {
        ...
        this.calcDelay = function(oldTime, newTime) {
            var delay;

            if (newTime > oldTime) {
                delay = newTime - oldTime;
            } else {
                delay = oldTime - newTime;
            }
            return delay;
        };
    }
